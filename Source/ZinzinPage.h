...
{
public:
    PadPoint (ZinzinPage& owner, int pointIndex);
...
private:
    ...
    int findOutputControlAt (juce::Point<float> localPosition) const;
    int findLengthControlAt (juce::Point<float> localPosition) const;
    std::array<juce::Point<float>, 3> getEqBandPositions() const;
    int findEqBandAt (juce::Point<float> localPosition) const;

    float dragStartY = 0.0f;
    float dragStartX = 0.0f;
    float dragStartValue = 0.0f;
    float dragStartEqValue = 0.5f;
    float dragStartLengthValue = 0.5f;
    int draggedEqBand = -1;
    int draggedLengthControl = -1;
    int draggedOutputControl = -1;

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (PadPoint)
};

class ZinzinPage  : public juce::Component,
...
private:
    ...
    std::array<std::array<float, 3>, 8> pointEqValues {};
    std::array<std::array<float, 3>, 8> pointWarpValues {};
    std::array<std::array<float, 3>, 8> pointLengthValues {};
    std::array<bool, 8> pointOutputSolo {};
    std::array<bool, 8> pointOutputMute {};
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ZinzinPage)
};
